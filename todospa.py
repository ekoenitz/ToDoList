...
def db_to_view_task(row):
    ...
    id, d, dd, pd, s = row
    return {'taskid': id, 'taskdescription': d, 'duedate': dd, 'postdate': pd, 'status': db_to_view_status(s)}
...
def view_to_db_new_task(task):
    ...
    return (task['taskdescription'], task['taskduedate'], task['taskpostdate'], view_to_db_status(task['status']))
...
def db_get_tasks_by_status(filter):
    ...
    query = "SELECT id, task, duedate, postdate, status FROM todo "
    
    if filter == 0 or filter == "tbd":
        query += "WHERE status=0 OR status LIKE 'unknown'"
    elif filter == 1 or filter == "done":
...
def db_create_task(new_task):
    ...
    db = DaBa("todo")
    # construct query to retrieve rows from todo table
    query = "INSERT INTO todo (task, duedate, postdate, status) Values('{td}', '{dd}', '{pd}', {status})"
    query = query.format(td=new_task[0], dd=new_task[1], pd=new_task[2], status=(new_task[3]))
    # execute query
    print("Query: " + query)
    db.que(query)
    result = db.new_id()
...
def db_get_task(taskid):
    ''' retrieves record with id '''
    print(taskid)
    db = DaBa("todo")
    query = "SELECT * FROM todo WHERE id={taskid}".format(taskid=taskid)
    print("in db_get_task, query: ")
    print(query)
    db.que("SELECT * FROM todo WHERE id LIKE ?", (taskid))
    result = db.one()
    db.close()
    return result
...
def view_updated_task(updated_task):
    ''' create new task:
        store task in db
        retrieve, convert to view, and return stored task
    '''
    ...
    # confirming record was correctly added
    row = db_get_task(updated_task['taskID'])
    result = db_to_view_task(row) if row else {'error': "task insertion failed"}
    return result
...
def update_task():
    ''' record edited task '''
    updated_task = unpack_task(request)
    print(updated_task)
    task = view_updated_task(updated_task)
    return pack_task(task)
